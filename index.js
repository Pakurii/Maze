...
document.addEventListener('keydown', (KeyboardEvent) => {
	...
	if (KeyboardEvent.code === 'KeyW') {
		Body.setVelocity(ball, { x, y: Math.max(y - 3, -speedLimit) });
		console.log(KeyboardEvent.code);
	}
	if (KeyboardEvent.code === 'KeyD') {
		Body.setVelocity(ball, { x: Math.min(x + 3, speedLimit), y });
	}
	if (KeyboardEvent.code === 'KeyS') {
		Body.setVelocity(ball, { x, y: Math.min(y + 3, speedLimit) });
	}
	if (KeyboardEvent.code === 'KeyA') {
		Body.setVelocity(ball, { x: Math.max(x - 3, -speedLimit), y });
	}
});
